...
impl<'a> ScanIteratorInner for ScanIterator<'a> {
    fn next_row(&mut self) -> Option<DBRow> {
        if self.done {
            return None;
        }
        let (key, value) = self.iter.next()?.expect("valid iterator");
...
pub struct TxIndexScanIterator<T: ScanIteratorInner + Sized> {
    iter: T,
}

impl<T: ScanIteratorInner + Sized> TxIndexScanIterator<T> {
    pub fn new(iter: T) -> TxIndexScanIterator<T> {
        TxIndexScanIterator { iter }
    }
}
impl<T: ScanIteratorInner + Sized> Iterator for TxIndexScanIterator<T> {
    type Item = DBRow;
...
pub trait ScanIteratorInner {
    fn next_row(&mut self) -> Option<DBRow>;
}

#[derive(Debug)]
pub struct RocksDBKVQCDB {
    db: rocksdb::DB,
}



pub trait TxIndexCDB {
    fn full_compaction(&self);
    fn enable_auto_compaction(&self);
    fn iter_scan(&self, prefix: &[u8]) -> ScanIterator;
    fn iter_scan_from(&self, prefix: &[u8], start_at: &[u8]) -> ScanIterator;
...
impl RocksDBKVQCDB {
    ...
    pub fn open(path: &Path, light_mode: bool) -> RocksDBKVQCDB {
        ...
        let db = RocksDBKVQCDB {
            db: rocksdb::DB::open(&db_opts, path).expect("failed to open RocksDB"),
        };
        db.verify_compatibility(light_mode);
        db
    }

    fn verify_compatibility(&self, light_mode: bool) {
        let mut compatibility_bytes = DB_VERSION.to_le_bytes().to_vec();
